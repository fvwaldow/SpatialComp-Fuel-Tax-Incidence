...
* SECOND INTERVENTION
*-------------------------------------------------------------------------------
*cd "X:\prj-avr\!_Analysen\vonWaldow_TaxIncidence_Market\Two_Stage_RDiT_Revision\SECOND_e10"
...
log using "BOOT_SECOND_E10", replace
...
est clear
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_season_file, replace): second_e10_season
	estimates save second_e10_season , replace
	* Supply & Seasonality
		program second_e10_sup
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new) saving(second_e10_sup_file, replace): second_e10_sup
	estimates save second_e10_sup , replace
	* Demand & Seasonality
		program second_e10_dem
		...
		xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new) saving(second_e10_sem_file, replace): second_e10_dem
	estimates save second_e10_dem , replace
	* Supply & Demand & Seasonality
	capture program drop _all
		program second_e10_allcov
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_allcov_file, replace): second_e10_allcov
	estimates save second_e10_allcov , replace
	esttab using "Main_e10_second_boot.rtf", replace b(2) se(2) ar2 label star(* 0.10 ** 0.05 *** 0.01) title(Main specification e10 second Intervention) nomtitles addnotes(Notes follow)
...
xtile hhi_km_quantile = hhi_4km, nquantiles(4)
	*Vertical Integration:
	capture program drop _all
		program second_e10_vert_int
			reghdfe mean_e10 $var_sup $var_dem $var_seas , absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.brand_vert_int [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h)))
			drop res_mean_e10 
		end	
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_vert_int_file, replace): second_e10_vert_int
	estimates save second_e10_vert_int , replace
	* Region Type:
	capture program drop _all
		program second_e10_reg_typ
			reghdfe mean_e10 $var_sup $var_dem $var_seas , absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.urban_rural_cat [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h)))
			drop res_mean_e10 
		end	
	xtset, clear
	...
	* Distance next competitor:
	capture program drop _all
		program second_e10_dist_comp
			reghdfe mean_e10 $var_sup $var_dem $var_seas , absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.d_comp [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h)))
			drop res_mean_e10 
		end	
	xtset, clear
	...
	* HHI Quartiles (4km):
		...
		program second_e10_hhi_4km
			reghdfe mean_e10 $var_sup $var_dem $var_seas , absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.hhi_km_quantile [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h)))
			drop res_mean_e10 
		end	
	xtset, clear
...
scalar scal_tax_mean_e10 = 1/35.16 * 100 // (1/+-35.16)*100 = +-2.844141069
	...
	* OLIGOPOLISTIC PLAYER
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_hhi_vert_1_file, replace): second_e10_hhi_vert_1
	ereturn list
	estimates store E_1_second_e10
	estimates save second_e10_hhi_vert_1, replace
	scalar scal_tankrabatt_1 = _b[1.tankrabatt_2]
	scalar scal_tankrabatt_1_per = abs(_b[1.tankrabatt_2])*scal_tax_mean_e10
	
	* OTHER INTEGRATED PLAYER
	capture program drop _all
		program second_e10_hhi_vert_2 
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_hhi_vert_2_file, replace): second_e10_hhi_vert_2
	ereturn list
	estimates store E_2_second_e10
	estimates save second_e10_hhi_vert_2, replace
	scalar scal_tankrabatt_2 = _b[1.tankrabatt_2]
	scalar scal_tankrabatt_2_per = abs(_b[1.tankrabatt_2])*scal_tax_mean_e10
	
	* INDEPENDENT PLAYER
	capture program drop _all
		program second_e10_hhi_vert_3 
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_hhi_vert_3_file, replace): second_e10_hhi_vert_3
	ereturn list
	estimates store E_3_second_e10
	estimates save second_e10_hhi_vert_3 , replace
	scalar scal_tankrabatt_3 = _b[1.tankrabatt_2]
	scalar scal_tankrabatt_3_per = abs(_b[1.tankrabatt_2])*scal_tax_mean_e10
		esttab using "HHI_VERT_e10_second_boot.rtf", replace b(2) se(2) ar2 label star(* 0.10 ** 0.05 *** 0.01) title(Heterogeneous Effects second Intervention) nomtitles addnotes(Notes follow)
	
coefplot (E_1_second_e10, transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_1) recast(bar) base(scal_tankrabatt_1) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(oligopolistic)) /// 
		 (E_2_second_e10, transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_2) recast(bar) base(scal_tankrabatt_2) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(other integrated)) /// 
		 (E_3_second_e10, transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_3) recast(bar) base(scal_tankrabatt_3) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(independent)), /// 
												  ...
												  scale(1.3) ysize(5cm) xsize(8cm) name(e10_brand_hhi_second, replace)
	graph save "HHI_VERT_second_e10", replace
coefplot (E_1_second_e10,  rescale(2.844141069) transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_1_per) recast(bar) base(scal_tankrabatt_1_per) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(oligopolistic)) /// 
		 (E_2_second_e10, rescale(2.844141069) transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_2_per) recast(bar) base(scal_tankrabatt_2_per) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(other integrated)) /// 
		 (E_3_second_e10, rescale(2.844141069) transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_3_per) recast(bar) base(scal_tankrabatt_3_per) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(independent)), /// 
												  ...
												  scale(1.3) ysize(5cm) xsize(8cm) name(e10_brand_hhi_second_per, replace)
	graph save "HHI_VERT_second_e10_per", replace
	graph export "HHI_VERT_second_e10_per.emf", replace
...
est clear
	* 3km RADIUS
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_hhi_rad_3_file, replace): second_e10_hhi_rad_3 
	ereturn list
	estimates store E3_second_e10_hhi_rad_3
	estimates save E3_second_e10_hhi_rad_3, replace
	scalar scal_tankrabatt_3 = _b[1.tankrabatt_2]
	scalar scal_tankrabatt_3_per = abs(_b[1.tankrabatt_2])*scal_tax_mean_e10
	
	* 4km RADIUS
	estimates use "E4_second_e10_hhi_rad_4"
	eststo: estimates replay "E4_second_e10_hhi_rad_4"
	estimates store E4_second_e10_hhi_rad_4
	scalar scal_tankrabatt_4 = _b[1.tankrabatt_2]
	scalar scal_tankrabatt_4_per = abs(_b[1.tankrabatt_2])*scal_tax_mean_e10
	
	* 5km RADIUS
	drop hhi_km_quantile
	xtile hhi_km_quantile = hhi_5km, nquantiles(4)
	capture program drop _all
		program second_e10_hhi_rad_5 
			reghdfe mean_e10 $var_sup $var_dem $var_seas , absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.hhi_km_quantile [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h)))
			drop res_mean_e10 
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_hhi_rad_5_file, replace): second_e10_hhi_rad_5 
	ereturn list
	estimates store e10_second_e10_hhi_rad_5
	estimates save e10_second_e10_hhi_rad_5, replace
	scalar scal_tankrabatt_5 = _b[1.tankrabatt_2]
	scalar scal_tankrabatt_5_per = abs(_b[1.tankrabatt_2])*scal_tax_mean_e10
...
coefplot (E3_second_e10_hhi_rad_3, transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_3) recast(bar) base(scal_tankrabatt_3) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(3km Radius)) /// 
		 (E4_second_e10_hhi_rad_4, transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_4) recast(bar) base(scal_tankrabatt_4) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(4km Radius)) /// 
		 (e10_second_e10_hhi_rad_5, transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_5) recast(bar) base(scal_tankrabatt_5) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(5km Radius)), /// 
												  vertical /// 
												  keep(1.tankrabatt_2 1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile) /// 
												  legend(pos(6) col(3)) xlabel(1 "({it:Ref.}) Q1" 2 "Q2" 3 "Q3" 4 "Q4") /// 
												  ytitle("Tax Pass-Through E10 (ct/l)") xtitle("HHI Quartiles") /// 
												  ...
												  scale(1.3) ysize(5cm) xsize(8cm) name(e10_sens_hhi_second, replace)
	graph save "HHI_SENS_second_e10", replace
coefplot (E3_second_e10_hhi_rad_3,  rescale(2.844141069) transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_3_per) recast(bar) base(scal_tankrabatt_3_per) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(3km Radius)) /// 
		 (E4_second_e10_hhi_rad_4, rescale(2.844141069) transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_4_per) recast(bar) base(scal_tankrabatt_4_per) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(4km Radius)) /// 
		 (e10_second_e10_hhi_rad_5, rescale(2.844141069) transform(1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile:= @+scal_tankrabatt_5_per) recast(bar) base(scal_tankrabatt_5_per) barwidth(0.2) lwidth(medthick) finten(40) ciopts(recast(rcap) lwidth(medthick)) citop label(5km Radius)), /// 
												  vertical /// 
												  keep(1.tankrabatt_2 1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile) /// 
												  legend(pos(6) col(3)) xlabel(1 "({it:Ref.}) Q1" 2 "Q2" 3 "Q3" 4 "Q4") /// 
												  ytitle("Tax Pass-Through E10 (%)") xtitle("HHI Quartiles") /// 
												  ...
												  scale(1.3) ysize(5cm) xsize(8cm) name(e10_sens_hhi_second_per, replace)
	graph save "HHI_SENS_second_e10_per", replace	
	graph export "HHI_SENS_second_e10_per.emf", replace
...
est clear
	...
	* 336 hour bandwidth (9 hours donut) siehe Average Effects
	estimates use "second_e10_allcov"
	eststo: estimates replay "second_e10_allcov"
	...
	* 384 hour (16 day) bandwidth
	...
	estimates save second_16days, replace
		esttab using "ROBUST_e10_second_boot.rtf", replace b(2) se(2) ar2 label star(* 0.10 ** 0.05 *** 0.01) title(Heterogeneous Effects second Intervention) nomtitles addnotes(Notes follow)
...
est clear	
	* ARAL
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_ARAL, replace): second_e10_ARAL
	estimates save second_e10_ARAL , replace
	* SHELL
	capture program drop _all
		program second_e10_SHELL
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_SHELL, replace): second_e10_SHELL
	estimates save second_e10_SHELL , replace
	* TOTALENERGIES
	capture program drop _all
		program second_e10_TOTALEnergies
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_TOTALEnergies, replace): second_e10_TOTALEnergies
	estimates save second_e10_TOTALEnergies , replace
	* ESSO
	capture program drop _all
		program second_e10_ESSO
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_ESSO, replace): second_e10_ESSO
	estimates save second_e10_ESSO , replace
	* JET
	capture program drop _all
		program second_e10_JET
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_JET, replace): second_e10_JET
	estimates save second_e10_JET , replace
	* BFT
	capture program drop _all
		program second_e10_BFT
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_BFT, replace): second_e10_BFT
	estimates save second_e10_BFT , replace
	* AVIA
	capture program drop _all
		program second_e10_AVIA
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_AVIA, replace): second_e10_AVIA
	estimates save second_e10_AVIA , replace
	esttab using "BRAND_e10_second_boot.rtf", replace b(2) se(2) ar2 label star(* 0.10 ** 0.05 *** 0.01) title(Main specification e10 second Intervention) nomtitles addnotes(Notes follow)
...
est clear	
	* Average Effect
	capture program drop _all
		program second_e10_allcov_nohour
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_allcov_file, replace): second_e10_allcov_nohour
	estimates save second_e10_nohour , replace
	* HHI Quartiles (4km):
	capture program drop _all
		program second_e10_hhi_4km_nohour
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.hhi_km_quantile [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h)))
			drop res_mean_e10 
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_hhi_4km_file, replace): second_e10_hhi_4km_nohour
	estimates save second_e10_nohour_hhi , replace
	* Average Effect: Morning Peak (05:00 to 10:59)
	capture program drop _all
		program second_e10_nohour_5_10
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2) [aw=kernel_wgt] ///
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_nohour_5_10_file, replace): second_e10_nohour_5_10
	estimates save second_e10_nohour_5_10, replace
	* Average Effect: Mid Day (11:00 to 16:59)
	capture program drop _all
		program second_e10_nohour_11_16
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2) [aw=kernel_wgt] ///
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_nohour_11_16_file, replace): second_e10_nohour_11_16
	estimates save second_e10_nohour_11_16, replace
	* Average Effect: Evening Off-Peak (17:00 to 22:59)
	capture program drop _all
		program second_e10_nohour_17_22
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2) [aw=kernel_wgt] ///
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_nohour_17_22_file, replace): second_e10_nohour_17_22
	estimates save second_e10_nohour_17_22, replace
	* Average Effect: Night (23:00 to 04:59)
	capture program drop _all
		program second_e10_nohour_23_4
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2) [aw=kernel_wgt] ///
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_nohour_23_4_file, replace): second_e10_nohour_23_4
	estimates save second_e10_nohour_23_4, replace
	* HHI Quartiles (4km): Morning Peak (05:00 to 10:59)
	capture program drop _all
		program second_e10_nohour_5_10_hhi
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.hhi_km_quantile [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h))) & inrange(hour,5,10)
			drop res_mean_e10 
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_nohour_5_10_file_hhi, replace): second_e10_nohour_5_10_hhi
	estimates save second_e10_nohour_5_10_hhi, replace
	* HHI Quartiles (4km): Mid Day (11:00 to 16:59)
	capture program drop _all
		program second_e10_nohour_11_16_hhi
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.hhi_km_quantile [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h))) & inrange(hour,11,16)
			drop res_mean_e10 
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_nohour_11_16_file_hhi, replace): second_e10_nohour_11_16_hhi
	estimates save second_e10_nohour_11_16_hhi, replace
	* HHI Quartiles (4km): Evening Off-Peak (17:00 to 22:59)
	capture program drop _all
		program second_e10_nohour_17_22_hhi
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.hhi_km_quantile [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h))) & inrange(hour,17,22)
			drop res_mean_e10 
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_nohour_17_22_file_hhi, replace): second_e10_nohour_17_22_hhi
	estimates save second_e10_nohour_17_22_hhi, replace
	* HHI Quartiles (4km): Night (23:00 to 04:59)
	capture program drop _all
		program second_e10_nohour_23_4_hhi
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.hhi_km_quantile [aw=kernel_wgt] ///
				if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h))) & (inrange(hour,0,4) | hour == 23)
			drop res_mean_e10 
	...
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_nohour_23_4_file_hhi, replace): second_e10_nohour_23_4_hhi
	estimates save second_e10_nohour_23_4_hhi, replace
	esttab using "nohour_e10_second_boot.rtf", replace b(2) se(2) ar2 label star(* 0.10 ** 0.05 *** 0.01) keep(1.tankrabatt_2 1.tankrabatt_2#2.hhi_km_quantile 1.tankrabatt_2#3.hhi_km_quantile 1.tankrabatt_2#4.hhi_km_quantile _cons) title(NO hourly FE e10 second Intervention) nomtitles addnotes(Notes follow)

*################### HHI and VERT-INT (one regression) ########################*
est clear	
	* Average Effect
	capture program drop _all
		program second_e10_het_included
			reghdfe mean_e10 $var_sup $var_dem i.week_days, absorb( station_id) resid(res_mean_e10)
			reg res_mean_e10 1.tankrabatt_2##(c.hour_to_treat_2##c.hour_to_treat_2)##i.hhi_km_quantile##i.brand_vert_int [aw=kernel_wgt] ///
									if (inrange(hour_to_treat_2,scalar(-h),scalar(db)) | inrange(hour_to_treat_2,scalar(da),scalar(h)))
			drop res_mean_e10 
		end			
	xtset, clear
	eststo: bootstrap, $boot_rep cluster(station_id) idcluster(id_new)  saving(second_e10_het_included, replace): second_e10_het_included
	estimates save second_e10_het_included , replace
	esttab using "hhi_vert_tog_e10_second_boot.rtf", replace b(2) se(2) ar2 label star(* 0.10 ** 0.05 *** 0.01) /// 
	keep(1.tankrabatt_2 /// 
		 ...
		 _cons) title(HHI and VERT e10 second Intervention) nomtitles addnotes(Notes follow)
